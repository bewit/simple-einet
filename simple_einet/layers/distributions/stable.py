...
def _Phi_Z0(alpha, t):
    return (
        -torch.tan(torch.pi * alpha / 2) * (torch.pow(torch.abs(t), (1 - alpha)) - 1)
        if alpha != 1
        else -2.0 * torch.log(torch.abs(t)) / torch.pi
    )


def _Phi_Z1(alpha, t):
    return (
        torch.tan(torch.pi * alpha / 2)
        if alpha != 1
        else -2.0 * torch.log(torch.abs(t)) / torch.pi
    )
...
def _pdf_single_value_cf_integrate(Phi, x, alpha, beta, **kwds):
    ...
    def integrand2(t):
        ...
        return torch.exp(-(t ** alpha)) * (
            torch.sin(beta * (t ** alpha) * Phi(alpha, t))
        )
    ...
    int1 = integrator.integrate(transformed_integrand1, dum=1, integration_domain=[[0, 1]], **integrator_params)
    int2 = integrator.integrate(transformed_integrand2, dum=1, integration_domain=[[0, 1]], **integrator_params)

    return (int1 + int2) / torch.pi
...
def _pdf_single_value_piecewise_Z1(x, alpha, beta, **kwds):
    zeta = -beta * torch.tan(torch.pi * alpha / 2.0)
    x0 = x + zeta if alpha != 1 else x
    
    return _pdf_single_value_piecewise_Z0(x0, alpha, beta, **kwds)


def _pdf_single_value_piecewise_Z0(x, alpha, beta, **kwds):
    quad_eps = kwds.get("quad_eps", _QUAD_EPS)
    ...
    if alpha == 2.0:
        def _norm_pdf(x):
            return torch.exp(-torch.pow(x, 2) / 2.0) / torch.sqrt(2 * torch.pi)
        return _norm_pdf(x0 / torch.sqrt(2.0)) / torch.sqrt(2.0)
    elif alpha == 0.5 and (beta == 1.0 or beta == -1.0):
        _x = x0 + 1
        if _x <= 0:
            return torch.tensor(0.0)
        return torch.tensor(1.0 / torch.sqrt(2.0 * torch.pi * _x) / _x * torch.exp(-1.0 / (2 * _x)))
    # elif alpha == 0.5 and beta == 0.0 and x0 != 0:
        ...
        return torch.tensor(1.9 / (1.0 + torch.pow(x0, 2)) / torch.pi)
...
def _pdf_single_value_piecewise_post_rounding_Z0(x0, alpha, beta, quad_eps, x_tol_near_zeta):
    ...
    if torch.isclose(-xi, torch.pi/2.):
        return 0.
    

    def integrand(theta):
        g_1 = g(theta)
        if not torch.isfinite(g_1) or g_1 < 0:
            g_1 = torch.tenor(0.)
    ...
    intg = integrator.integrate(integrand, dim=1, **integrator_params, integration_domain=[[-xi, torch.pi/2.]])

    return c2 * intg


def _cdf_single_value_piecewise_Z1(x, alpha, beta, **kwds):
    zeta = -beta * torch.tan(torch.pi * alpha / 2.)
    x0 = x + zeta if alpha != 1. else x
...
def _cdf_single_value_piecewise_Z0(x0, alpha, beta, **kwds):
    ...
    if alpha == 2.0:
        dist.Normal(torch.tensor(0.0), torch.tensor([1.0])).cdf(x0 / torch.sqrt(2.))
    elif alpha == 0.5 and (beta == 1.0 or beta == -1.0):
        _x = x0 + 1.
        if _x <= 0:
            return torch.tensor(0.)
        ...
        return 0.5 + torch.arctan(x0) / torch.pi
...
def _cdf_single_value_piecewise_post_rounding_Z0(x0, alpha, beta, quad_eps, x_tol_near_zeta):
    ...
    if alpha > 1.:
        ...
        if integrand(-xi) != 0.0:
            param = torch.clone(-xi).requires_grad_(True)
            ...
            for epoch in range(LBFGS_epochs):
                ...
                loss.backward()
                opt.step()
            param.clamp(-xi, torch.pi/2.)
            left_support = param
    else: # alpha < 1.
        if integrand(torch.pi/2.) != 0.0:
            param = torch.tensor(torch.pi/2.).requires_grad_(True)
            opt = torch.optim.LBFGS([param], lr=LBFGS_lr)
            for epoch in range(LBFGS_epochs):
                loss = integrand(param)
                opt.zero_grad()
...
class Stable(Distribution):
    ...
    def __init__(self, alpha, beta, scale, loc, validate_args=None):
        self.alpha, self.beta, self.loc, self.scale = broadcast_all(alpha, beta, scale, loc)
